import { POLICY_VERSION } from './testing-policy.js';
export function cloudStore(db) {
  return {
    ...
    async add(item) { return db.prepare('INSERT OR REPLACE INTO updates(id,tech,published,payload) VALUES (?,?,?,?)').bind(item.id,item.tech,item.published,JSON.stringify(item)).run(); },
    async has(id) { const row = await db.prepare('SELECT payload FROM updates WHERE id=?').bind(id).first(); return row ? JSON.parse(row.payload).policyVersion === POLICY_VERSION : false; },
